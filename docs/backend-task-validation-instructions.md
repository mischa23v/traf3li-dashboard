...
## 7. Solo Lawyer vs Firm Member (IMPORTANT)

The frontend now distinguishes between **solo lawyers** and **firm members**:

### How It Works

| User Type | Detection | `assignedTo` Field |
|-----------|-----------|-------------------|
| **Solo Lawyer** | `user.firmId` is `null`/`undefined` | Hidden in UI, not sent |
| **Firm Member** | `user.firmId` has value | Shown, can assign to team members |

### Backend Implications

1. **Solo Lawyer Task Creation:**
   - `assignedTo` field will be omitted or empty
   - Backend should auto-assign to the task creator
   - `GET /lawyers/team` endpoint may return empty array for solo lawyers

2. **Firm Member Task Creation:**
   - `assignedTo` can be any valid team member ID
   - `GET /lawyers/team` should return firm's team members
...
if (!payload.assignedTo) {
  // Auto-assign to creator for solo lawyers
...
| Endpoint | Solo Lawyer | Firm Member |
|----------|-------------|-------------|
| `GET /lawyers/team` | Return `[]` or `[self]` | Return all firm team members |
| `POST /tasks` | Accept without `assignedTo` | Accept with valid `assignedTo` |

---

## 8. Basic/Advanced Mode (Frontend Only)
