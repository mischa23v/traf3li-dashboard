...
import { SLADashboard, PriorityQueue } from '@/features/ml-scoring/components'
...
function MLSLAPage() {
  ...
  const topNav = [
    ...
    { title: t('mlScoring.nav.queue', 'Priority Queue'), href: '/dashboard/ml/queue', isActive: false },
    { title: t('mlScoring.nav.sla', 'SLA Dashboard'), href: '/dashboard/ml/sla', isActive: true },
  ]
    ...
    <>
      ...
      >
        <ProductivityHero
          ...
          title={t('mlScoring.sla.title', 'SLA Performance')}
          type="analytics"
        />

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <SLADashboard period="30d" />
          </div>
          <div>
            <PriorityQueue showFilters={false} limit={10} />
          </div>
        </div>
      </Main>
    </>
...
export const Route = createFileRoute('/_authenticated/dashboard/ml/sla')({
  component: MLSLAPage,
})
