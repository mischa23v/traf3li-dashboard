...
import {
  ...
  FileText, BarChart3, Clock, Shield, CheckSquare, Users
} from 'lucide-react'
import {
  reportStatusLabels,
...
export function TasksReportsCreateView() {
  ...
  const tasksCategories = categoriesBySection[ReportSection.TASKS]
  const tasksDataModules = dataModulesBySection[ReportSection.TASKS]
  ...
  const [reportCategory, setReportCategory] = useState<ReportCategory>(ReportCategory.TASKS)
  ...
  const [selectedDataModules, setSelectedDataModules] = useState<DataModule[]>([DataModule.TASKS])
  const [accessLevel, setAccessLevel] = useState<AccessLevel>(AccessLevel.TEAM)
  ...
  useEffect(() => {
    if (existingReport && isEditMode) {
      ...
      setAccessLevel(existingReport.accessControl?.accessLevel || AccessLevel.TEAM)
  ...
  const handleSubmit = async () => {
    const data: CreateReportInput = {
      ...
      reportSection: ReportSection.TASKS,
      reportCategory,
      reportType,
      reportFormat,
      outputFormats,
  ...
  return (
    <>
      ...
      <Main fluid={true} className="bg-[#f8f9fa] flex-1 w-full p-6 lg:p-8 space-y-8 rounded-tr-3xl shadow-inner border-r border-white/5 overflow-hidden font-['IBM_Plex_Sans_Arabic']">
        <ProductivityHero badge="الإنتاجية" title={isEditMode ? 'تعديل التقرير' : 'إنشاء تقرير جديد'} type="tasks-reports" listMode={true} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-navy">{isEditMode ? 'تعديل التقرير' : 'إنشاء تقرير إنتاجية جديد'}</h1>
                <p className="text-slate-500">{isEditMode ? 'تعديل بيانات التقرير' : 'إضافة تقرير جديد للإنتاجية والمهام'}</p>
              </div>
            </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={reportCode} onChange={(e) => setReportCode(e.target.value)} placeholder="مثال: TSK-RPT-001" className="h-11 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={reportName} onChange={(e) => setReportName(e.target.value)} placeholder="Task Summary Report" className="h-11 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-navy font-medium">اسم التقرير (عربي)</Label>
                    <Input value={reportNameAr} onChange={(e) => setReportNameAr(e.target.value)} placeholder="تقرير ملخص المهام" className="h-11 rounded-xl" />
                  </div>
                </div>
                <div className="space-y-2">
