...
import {
  ...
  Play, Download, Share2, Copy, Pause, Activity, Users,
  LineChart, Building2, Filter
} from 'lucide-react'
import {
  reportStatusLabels,
...
export function ReportsListView() {
  ...
  const getCategoryColor = (category: ReportCategory) => {
    const colors: Record<string, string> = {
      employee_data: 'bg-blue-100 text-blue-700',
  ...
  const topNav = [
    ...
    { title: 'التقارير والتحليلات', href: ROUTES.dashboard.hr.reports.list, isActive: true },
  ]
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-navy">{stats?.totalReports || 0}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-navy">{stats?.activeReports || 0}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-navy">{stats?.scheduledReports || 0}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-blue-800">توزيع التقارير</p>
                      <p className="text-xs text-blue-600">التقارير حسب الفئة</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-sm">
                    {stats?.byCategory && Object.entries(stats.byCategory).slice(0, 4).map(([category, count]) => (
                      <div key={category} className="text-center">
                        <p className="text-lg font-bold text-navy">{count as number}</p>
                        <p className="text-xs text-slate-500">{reportCategoryLabels[category as ReportCategory]?.ar || category}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <CardContent className="p-4">
                <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      onClick={() => navigate({ to: ROUTES.dashboard.hr.reports.new })}
            ...
            {isLoading ? (
              ...
              <Card className="rounded-2xl border-slate-100">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="mt-4 text-slate-500">لا توجد تقارير</p>
                  <Button
                    onClick={() => navigate({ to: ROUTES.dashboard.hr.reports.new })}
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-slate-500">
                                    {report.reportCode} - {report.reportNameAr || report.reportDescription?.slice(0, 50)}
                                  </p>
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                ...
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end">
                                    <DropdownMenuItem onClick={() => navigate({ to: ROUTES.dashboard.hr.reports.detail(report.reportId) })}>
                              ...
                              <div className="mt-4 pt-3 border-t border-slate-100">
                                <div className="flex items-center gap-4 text-sm">
                                  <div className="flex items-center gap-2 px-3 py-1 bg-purple-50 rounded-lg">
                                    ...
                                    <span className="text-purple-700">مجدول</span>
                                  </div>
                                  {report.scheduling.nextRunDate && (
                                    <span className="text-slate-500">
                                      التشغيل التالي: {new Date(report.scheduling.nextRunDate).toLocaleDateString('ar-SA')}
                                    </span>
                                  )}
                                  <span className="text-slate-500">|</span>
                                  <span className="text-slate-500">
                                    {report.scheduling.totalRuns} تشغيل ({report.scheduling.successfulRuns} ناجح)
                                  </span>
                                </div>
                              </div>
                            )}
                              ...
                              <div className="flex items-center gap-4 text-xs text-slate-500">
                                ...
                                {report.visualization?.charts && report.visualization.charts.length > 0 && (
                                  <span className="flex items-center gap-1 text-blue-600">
                                    ...
                                    {report.visualization.charts.length} رسم بياني
                                  </span>
                                )}
                              </div>
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => navigate({ to: ROUTES.dashboard.hr.reports.detail(report.reportId) })}
