...
import {
    ...
    Eye, Scale, FileText, CheckSquare, TrendingUp, PieChart,
...
export function CasePipelineSidebar({
    ...
    // Get current case type label
    const currentCaseTypeLabel = caseTypes.find(t => t.value === selectedCaseType)
        ...
        <div className="space-y-8 lg:col-span-1">
            ...
            <div className="bg-gradient-to-br from-emerald-900 to-slate-900 rounded-3xl p-6 shadow-xl shadow-emerald-900/20 ring-1 ring-white/10 relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-6 relative z-10">
                    <h3 className="font-bold text-lg text-white">{t('casePipeline.sidebar.quickActions', 'إجراءات سريعة')}</h3>
                </div>
                ...
                <div className="relative z-10 grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    {/* Tasks Button */}
                    <Button asChild variant="ghost" className="bg-white hover:bg-blue-50 text-blue-600 h-auto py-6 flex flex-col items-center justify-center gap-2 rounded-3xl transition-all duration-300 hover:scale-[1.02] shadow-lg shadow-white/10">
                        <Link to="/dashboard/tasks/list">
                            <CheckSquare className="h-6 w-6" aria-hidden="true" />
                            <span className="text-sm font-bold">{t('casePipeline.sidebar.tasks', 'المهام')}</span>
                        </Link>
                    </Button>

                    {/* Brainstorm Button */}
                    <Button asChild variant="ghost" className="bg-white hover:bg-purple-50 text-purple-600 h-auto py-6 flex flex-col items-center justify-center gap-2 rounded-3xl transition-all duration-300 hover:scale-[1.02] shadow-lg shadow-white/10">
                        <Link to="/dashboard/notion">
                            <FileText className="h-6 w-6" aria-hidden="true" />
                            <span className="text-sm font-bold">{t('casePipeline.sidebar.brainstorm', 'العصف الذهني')}</span>
                        </Link>
                    </Button>
                </div>
            </div>

            {/* CASE TYPE STATS WIDGET */}
            {analytics && (
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-bold text-navy">
                            {i18n.language === 'ar' ? currentCaseTypeLabel?.label : currentCaseTypeLabel?.labelEn}
                        </h3>
                        <Badge variant="outline" className="text-xs">
                            {analytics.totalCases} {t('casePipeline.sidebar.cases', 'قضية')}
                        </Badge>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-slate-600">{t('casePipeline.sidebar.won', 'كسب')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-slate-600">{t('casePipeline.sidebar.lost', 'خسارة')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden">
                                    <div
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-slate-600">{t('casePipeline.sidebar.settled', 'تسوية')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden">
                                    <div
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-slate-600">{t('casePipeline.sidebar.ongoing', 'جاري')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden">
                                    <div
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100">
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-slate-600">{t('casePipeline.sidebar.winRate', 'معدل الفوز')}</span>
                            <span className="text-lg font-bold text-emerald-600">{analytics.winRate}%</span>
                        </div>
                    </div>

                    {/* Alerts */}
                    {(analytics.staleCases > 0 || analytics.urgentCases > 0) && (
                        <div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
                            {analytics.staleCases > 0 && (
                                <div className="flex items-center gap-2 text-orange-600 text-sm">
                                    <AlertTriangle className="h-4 w-4" />
                                    <span>{analytics.staleCases} {t('casePipeline.sidebar.needsAttention', 'قضية تحتاج متابعة')}</span>
                                </div>
                            )}
                            {analytics.urgentCases > 0 && (
                                <div className="flex items-center gap-2 text-red-600 text-sm">
                                    <AlertCircle className="h-4 w-4" />
                                    <span>{analytics.urgentCases} {t('casePipeline.sidebar.urgent', 'قضية عاجلة')}</span>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <div className="flex justify-between items-center mb-6 relative z-10">
                    <div className="flex bg-[#033a2d] p-1 rounded-xl">
                        <button
                            ...
                            aria-label={t('casePipeline.sidebar.calendar', 'التقويم')}
                            className={cn(
                        ...
                        >
                            {t('casePipeline.sidebar.calendar', 'التقويم')}
                        </button>
                        <button
                            onClick={() => setActiveTab('notifications')}
                            aria-label={t('casePipeline.sidebar.notifications', 'التنبيهات')}
                            className={cn(
                                "px-4 py-2 text-sm font-bold rounded-lg transition-all duration-200",
                        ...
                        >
                            {t('casePipeline.sidebar.notifications', 'التنبيهات')}
                        </button>
                    </div>
                    {activeTab === 'calendar' && (
                        ...
                        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="space-y-6 relative min-h-[200px]">
                                {isCalendarLoading ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center py-8 text-slate-500">
                                        ...
                                        <p className="text-xs font-medium">{t('casePipeline.sidebar.noEvents', 'لا توجد أحداث لهذا اليوم')}</p>
                                    </div>
                                ) : (
                                    <>
                                        {/* Vertical Line */}
                            ...
                            <Button asChild variant="ghost" className="w-full mt-6 text-slate-500 hover:text-emerald-700 hover:bg-emerald-50 group cursor-pointer">
                                <Link to="/dashboard/calendar">
                                    <span>{t('casePipeline.sidebar.viewFullSchedule', 'عرض الجدول الكامل')}</span>
                        ...
                        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300 min-h-[200px]">
                            {isRemindersLoading ? (
                                ...
                                <div className="flex flex-col items-center justify-center py-8 text-slate-500">
                                    ...
                                    <p className="text-xs font-medium">{t('casePipeline.sidebar.noReminders', 'لا توجد تنبيهات قادمة')}</p>
                                </div>
                            ) : (
                                <>
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        ...
                                                        {isOverdue && (
                                                            <Badge variant="destructive" className="text-[10px] px-1.5 py-0">
                                                                {t('casePipeline.sidebar.overdue', 'متأخر')}
                                                            </Badge>
                                                        )}
                                                    </div>
                                                </div>
                            ...
                            <Button asChild variant="ghost" className="w-full text-xs text-slate-500 hover:text-emerald-600 hover:bg-emerald-50">
                                <Link to="/dashboard/tasks/reminders">
                                    {t('casePipeline.sidebar.viewAllReminders', 'عرض كل التنبيهات')}
                                </Link>
                            </Button>
                        </div>
                    )}
