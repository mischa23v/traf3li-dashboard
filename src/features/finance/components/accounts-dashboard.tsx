...
export default function AccountsDashboard({ }: AccountsDashboardProps) {
    ...
    useEffect(() => {
        ...
        if (!wizardProgress) {
            ...
            navigate({ to: ROUTES.dashboard.finance.setupWizard })
            return
        }
    ...
    const financialSummary = useMemo(() => {
        ...
        // Handle both { invoices: [] } and { data: [] } response structures
        const invoiceArray = invoicesData?.invoices ?? invoicesData?.data ?? []
        const outstanding = invoiceArray.reduce((sum: number, inv: any) => sum + (inv.balanceDue || inv.totalAmount || 0), 0)
    ...
    const topNav = [
        { title: 'نظرة عامة', href: ROUTES.dashboard.finance.overview, isActive: true },
