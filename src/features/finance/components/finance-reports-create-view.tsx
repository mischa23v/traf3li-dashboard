...
import {
  ...
  FileText, BarChart3, Clock, Shield, DollarSign, Users
} from 'lucide-react'
import {
  reportStatusLabels,
...
export function FinanceReportsCreateView() {
  ...
  const financeDataModules = dataModulesBySection[ReportSection.FINANCE]
  ...
  const [reportCategory, setReportCategory] = useState<ReportCategory>(ReportCategory.INVOICES)
  ...
  const [usageFrequency, setUsageFrequency] = useState<UsageFrequency>(UsageFrequency.MONTHLY)
  ...
  const [selectedDataModules, setSelectedDataModules] = useState<DataModule[]>([DataModule.INVOICES])
  const [accessLevel, setAccessLevel] = useState<AccessLevel>(AccessLevel.FINANCE)
  ...
  useEffect(() => {
    if (existingReport && isEditMode) {
      ...
      setAccessLevel(existingReport.accessControl?.accessLevel || AccessLevel.FINANCE)
  ...
  return (
    <>
      ...
      <Main fluid={true} className="bg-[#f8f9fa] flex-1 w-full p-6 lg:p-8 space-y-8 rounded-tr-3xl shadow-inner border-r border-white/5 overflow-hidden font-['IBM_Plex_Sans_Arabic']">
        <ProductivityHero badge="المالية" title={isEditMode ? 'تعديل التقرير' : 'إنشاء تقرير مالي'} type="finance-reports" listMode={true} />

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="icon" className="rounded-xl hover:bg-white" onClick={() => navigate({ to: '/dashboard/finance/reports' })}>
                <ArrowRight className="h-5 w-5" />
              </Button>
              <div>
                <h1 className="text-2xl font-bold text-navy">{isEditMode ? 'تعديل التقرير' : 'إنشاء تقرير مالي جديد'}</h1>
                <p className="text-slate-500">{isEditMode ? 'تعديل بيانات التقرير' : 'إضافة تقرير جديد للمالية'}</p>
              </div>
            </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={reportCode} onChange={(e) => setReportCode(e.target.value)} placeholder="مثال: FIN-RPT-001" className="h-11 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={reportName} onChange={(e) => setReportName(e.target.value)} placeholder="Invoice Summary Report" className="h-11 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-navy font-medium">اسم التقرير (عربي)</Label>
                    <Input value={reportNameAr} onChange={(e) => setReportNameAr(e.target.value)} placeholder="تقرير ملخص الفواتير" className="h-11 rounded-xl" />
                  </div>
                </div>
                <div className="space-y-2">
            ...
            <div className="flex items-center justify-end gap-4">
              <Button variant="outline" onClick={() => navigate({ to: '/dashboard/finance/reports' })} className="rounded-xl">إلغاء</Button>
              <Button onClick={handleSubmit} disabled={!reportCode || !reportName || isPending} className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl px-8">
                {isPending ? <>جاري الحفظ...</> : (
                  <><CheckCircle className="w-4 h-4 ms-2" aria-hidden="true" />{isEditMode ? 'حفظ التعديلات' : 'إنشاء التقرير'}</>
                )}
              </Button>
            </div>
          </div>
