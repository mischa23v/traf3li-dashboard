...
import {
  ...
  usePauseSchedule, useResumeSchedule, useExecutionHistory,
...
import {
  Search, Bell, ArrowRight, Calendar, AlertCircle, Loader2, FileText,
  Trash2, Edit, MoreHorizontal, Play, Download, Copy, Share2, Star, StarOff,
  Clock, Shield, BarChart3, PieChart, Database, Users, Eye, Pause, Activity,
  CheckCircle, XCircle, LineChart, UserPlus, MessageCircle, Mail, TrendingUp
} from 'lucide-react'
import {
...
export function CrmReportsDetailsView() {
  ...
  const handleDuplicate = async () => {
    ...
    await duplicateMutation.mutateAsync({ id: reportId, newName: `${report.reportName} - نسخة` })
  }
  ...
  return (
    <>
      ...
      <Main fluid={true} className="bg-[#f8f9fa] flex-1 w-full p-6 lg:p-8 space-y-8 rounded-tr-3xl shadow-inner border-e border-white/5 overflow-hidden font-['IBM_Plex_Sans_Arabic']">
        <ProductivityHero badge="العملاء والتواصل" title="تفاصيل التقرير" type="crm-reports" listMode={true} />
            ...
            {isLoading ? (
              ...
              <>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h1 className="text-2xl font-bold text-navy">{report.reportNameAr || report.reportName}</h1>
                        <Badge className={getStatusColor(report.status)}>{reportStatusLabels[report.status]?.ar}</Badge>
                        {report.isFavorite && <Star className="w-5 h-5 text-amber-500 fill-amber-500" />}
                      </div>
                      <p className="text-slate-500">{report.reportCode} - {reportCategoryLabels[report.reportCategory]?.ar}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        {report.outputFormats.map((format) => (
                          <DropdownMenuItem key={format} onClick={() => handleExport(format)}>{outputFormatLabels[format]?.ar || format}</DropdownMenuItem>
                        ))}
                      </DropdownMenuContent>
                    </DropdownMenu>
                    <Button variant="outline" size="icon" className="rounded-xl" onClick={handleToggleFavorite}>
                      {report.isFavorite ? <Star className="w-4 h-4 text-amber-500 fill-amber-500" /> : <StarOff className="w-4 h-4" />}
                    </Button>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Card className="rounded-2xl border-slate-100"><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-blue-100 rounded-xl"><Play className="w-5 h-5 text-blue-600" /></div><div><p className="text-xs text-slate-500">مرات التشغيل</p><p className="text-lg font-bold text-navy">{report.runCount || 0}</p></div></div></CardContent></Card>
                  <Card className="rounded-2xl border-slate-100"><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-emerald-100 rounded-xl"><CheckCircle className="w-5 h-5 text-emerald-600" /></div><div><p className="text-xs text-slate-500">نجاح</p><p className="text-lg font-bold text-navy">{report.scheduling?.successfulRuns || 0}</p></div></div></CardContent></Card>
                  <Card className="rounded-2xl border-slate-100"><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-red-100 rounded-xl"><XCircle className="w-5 h-5 text-red-600" /></div><div><p className="text-xs text-slate-500">فشل</p><p className="text-lg font-bold text-navy">{report.scheduling?.failedRuns || 0}</p></div></div></CardContent></Card>
                  <Card className="rounded-2xl border-slate-100"><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-purple-100 rounded-xl"><Activity className="w-5 h-5 text-purple-600" /></div><div><p className="text-xs text-slate-500">متوسط الوقت</p><p className="text-lg font-bold text-navy">{report.performance?.performanceMetrics?.averageExecutionTime ? `${(report.performance.performanceMetrics.averageExecutionTime / 1000).toFixed(1)}ث` : '-'}</p></div></div></CardContent></Card>
                </div>
                ...
                <Tabs defaultValue="overview" className="w-full">
                    ...
                    <Card className="rounded-2xl border-slate-100">
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div><p className="text-xs text-slate-500">الاسم (عربي)</p><p className="font-medium">{report.reportNameAr || '-'}</p></div>
                          <div><p className="text-xs text-slate-500">الكود</p><p className="font-medium">{report.reportCode}</p></div>
                          <div><p className="text-xs text-slate-500">الإصدار</p><p className="font-medium">{report.version || '1.0'}</p></div>
                        </div>
                        {report.reportDescription && <div><p className="text-xs text-slate-500">الوصف</p><p className="text-sm text-slate-700">{report.reportDescription}</p></div>}
                      </CardContent>
                    </Card>

                    <Card className="rounded-2xl border-slate-100">
                      <CardHeader><CardTitle className="text-lg font-bold text-slate-800 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-blue-500" />التصنيف</CardTitle></CardHeader>
                      <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div><p className="text-xs text-slate-500">الفئة</p><Badge className="mt-1 bg-blue-100 text-blue-700">{reportCategoryLabels[report.reportCategory]?.ar}</Badge></div>
                          <div><p className="text-xs text-slate-500">النوع</p><Badge className="mt-1 bg-purple-100 text-purple-700">{reportTypeLabels[report.reportType]?.ar}</Badge></div>
                          <div><p className="text-xs text-slate-500">التنسيق</p><Badge className="mt-1 bg-emerald-100 text-emerald-700">{reportFormatLabels[report.reportFormat]?.ar}</Badge></div>
                          <div><p className="text-xs text-slate-500">الأهمية</p><Badge className="mt-1 bg-amber-100 text-amber-700">{criticalityLevelLabels[report.criticalityLevel]?.ar}</Badge></div>
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="rounded-2xl border-slate-100">
                      ...
                      <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div><p className="text-xs text-slate-500">الدور</p><p className="font-medium">{report.reportOwner.ownerRole || '-'}</p></div>
                          <div><p className="text-xs text-slate-500">القسم</p><p className="font-medium">{report.reportOwner.department || '-'}</p></div>
                          <div><p className="text-xs text-slate-500">التكرار</p><p className="font-medium">{usageFrequencyLabels[report.usageFrequency]?.ar}</p></div>
                        </div>
                      </CardContent>
                    </Card>
                  </TabsContent>
                  ...
                  <TabsContent value="structure" className="space-y-6 mt-6">
                    <Card className="rounded-2xl border-slate-100">
                      <CardHeader><CardTitle className="text-lg font-bold text-slate-800 flex items-center gap-2"><LineChart className="w-5 h-5 text-blue-500" />الأعمدة ({report.reportStructure?.columns?.length || 0})</CardTitle></CardHeader>
                      <CardContent>
                        {report.reportStructure?.columns && report.reportStructure.columns.length > 0 ? (
                          <div className="space-y-2">{report.reportStructure.columns.map((column, i) => (<div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl"><div><p className="font-medium">{column.columnNameAr || column.columnName}</p><p className="text-xs text-slate-500">{column.dataField}</p></div><div className="flex items-center gap-2"><Badge variant="outline" className="text-xs">{column.dataType}</Badge>{column.sortable && <Badge className="text-xs bg-blue-100 text-blue-700">قابل للفرز</Badge>}{column.filterable && <Badge className="text-xs bg-purple-100 text-purple-700">قابل للتصفية</Badge>}</div></div>))}</div>
                    ...
                    {report.scheduling?.scheduled ? (
                      <>
                        <Card className="rounded-2xl border-slate-100">
                          ...
                          <CardContent>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              <div><p className="text-xs text-slate-500">التكرار</p><p className="font-medium">{report.scheduling.recurrence?.frequency ? scheduleFrequencyLabels[report.scheduling.recurrence.frequency]?.ar : '-'}</p></div>
                              <div><p className="text-xs text-slate-500">تاريخ البدء</p><p className="font-medium">{report.scheduling.recurrence?.startDate ? new Date(report.scheduling.recurrence.startDate).toLocaleDateString('ar-SA') : '-'}</p></div>
                              <div><p className="text-xs text-slate-500">الوقت</p><p className="font-medium">{report.scheduling.recurrence?.time || '-'}</p></div>
                              <div><p className="text-xs text-slate-500">التشغيل التالي</p><p className="font-medium">{report.scheduling.nextRunDate ? new Date(report.scheduling.nextRunDate).toLocaleDateString('ar-SA') : '-'}</p></div>
                            </div>
                          </CardContent>
                        </Card>
                        <Card className="rounded-2xl border-slate-100">
                          <CardHeader><CardTitle className="text-lg font-bold text-slate-800 flex items-center gap-2"><Activity className="w-5 h-5 text-emerald-500" />إحصائيات التشغيل</CardTitle></CardHeader>
                          <CardContent>
                            <div className="grid grid-cols-3 gap-4">
                              <div className="text-center p-4 bg-slate-50 rounded-xl"><p className="text-3xl font-bold text-navy">{report.scheduling.totalRuns}</p><p className="text-sm text-slate-500">إجمالي التشغيل</p></div>
                              <div className="text-center p-4 bg-emerald-50 rounded-xl"><p className="text-3xl font-bold text-emerald-600">{report.scheduling.successfulRuns}</p><p className="text-sm text-slate-500">ناجح</p></div>
                              <div className="text-center p-4 bg-red-50 rounded-xl"><p className="text-3xl font-bold text-red-600">{report.scheduling.failedRuns}</p><p className="text-sm text-slate-500">فشل</p></div>
                            </div>
                            {report.scheduling.totalRuns > 0 && <div className="mt-4"><div className="flex items-center justify-between text-sm mb-2"><span>نسبة النجاح</span><span className="font-medium">{((report.scheduling.successfulRuns / report.scheduling.totalRuns) * 100).toFixed(1)}%</span></div><Progress value={(report.scheduling.successfulRuns / report.scheduling.totalRuns) * 100} className="h-2" /></div>}
                          </CardContent>
                        </Card>
                      </>
                    ) : (
