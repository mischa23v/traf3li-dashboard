...
export function WikiGlobalCreateView() {
  ...
  // Fetch cases for optional linking
  const { data: casesData, isLoading: casesLoading } = useCases({})
  const cases = casesData?.cases || []

  // Selected case for collections
  ...
  const topNav = [
    ...
    { title: isArabic ? 'مراجع والملاحظات' : 'References & Notes', href: '/dashboard/wiki' as any, isActive: false },
      ...
      <Main fluid={true} className="bg-[#f8f9fa] flex-1 w-full p-6 lg:p-8 space-y-8 rounded-tr-3xl shadow-inner border-r border-white/5 overflow-hidden font-['IBM_Plex_Sans_Arabic']">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div className="space-y-6">
              ...
              <CardContent className="p-4">
                ...
                <div className="relative mb-4">
                  ...
                  <Input
                    ...
                    className="h-10 rounded-xl border-slate-200 pr-10 focus:border-emerald-500 focus:ring-emerald-500"
                  />
                </div>
                    ...
                    {casesLoading ? (
                      ...
                      filteredCases.map((caseItem) => (
                        <div
                          ...
                          className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors ${selectedCaseId === caseItem._id
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {caseItem.caseNumber && (
                              <div className="text-xs text-slate-500">{caseItem.caseNumber}</div>
                            )}
                          </div>
                        </div>
                      ))
            ...
            {/* Collections Card (only if case selected) */}
            {selectedCaseId && (
              <Card className="border border-slate-100 shadow-sm rounded-2xl overflow-hidden">
                <CardHeader className="bg-white border-b border-slate-50 pb-4">
                  <CardTitle className="text-lg font-bold text-navy flex items-center gap-2">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* HERO CARD */}
            <ProductivityHero badge="المراجع والملاحظات" title={isArabic ? 'إنشاء صفحة جديدة' : 'Create New Page'} type="wiki" hideButtons={true} />
