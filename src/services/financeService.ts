...
const financeService = {
   ...
   */
  getInvestments: async (filters?: InvestmentFilters): Promise<InvestmentsResponse> => {
    try {
   ...
   * Update investment
   */
  updateInvestment: async (id: string, data: Partial<CreateInvestmentData>): Promise<Investment> => {
    try {
   ...
   */
  addInvestmentTransaction: async (investmentId: string, data: CreateTransactionPayload): Promise<InvestmentTransaction> => {
    try {
      const response = await apiClient.post(`/investments/${investmentId}/transactions`, data)
      return response.data.transaction || response.data.data
    } catch (error: any) {
  ...
  /**
   * Refresh investment price from market
   */
  refreshInvestmentPrice: async (investmentId: string): Promise<Investment> => {
    try {
      const response = await apiClient.post(`/investments/${investmentId}/refresh-price`)
      return response.data.investment || response.data.data
    } catch (error: any) {
   ...
   */
  refreshAllPrices: async (): Promise<{ updated: number; errors: number }> => {
    try {
      const response = await apiClient.post('/investments/refresh-prices')
      return response.data.result || response.data.data || { updated: 0, errors: 0 }
    } catch (error: any) {
  ...
  searchSymbols: async (query: string): Promise<SymbolSearchResult[]> => {
    try {
      const response = await apiClient.get('/investment-search/symbols', { params: { q: query } })
      return response.data.symbols || response.data.data || []
    } catch (error: any) {
  ...
  getQuote: async (symbol: string): Promise<StockQuote> => {
    try {
      const response = await apiClient.get('/investment-search/quote', { params: { symbol } })
      return response.data.quote || response.data.data
    } catch (error: any) {
   ...
   */
  sellInvestment: async (investmentId: string, data: { quantity: number; salePrice: number; saleDate: string; fees?: number }): Promise<Investment> => {
    try {
      const response = await apiClient.post(`/investments/${investmentId}/sell`, data)
      return response.data.investment || response.data.data
    } catch (error: any) {
...
export interface SymbolSearchResult {
  ...
  name: string
  nameEn: string
  type: 'stock' | 'etf' | 'reit' | 'mutual_fund'
  sector: string
  sectorEn: string
  market: string
}
...
export interface StockQuote {
  symbol: string
  price: number              // halalas
  change: number             // halalas
  changePercent: number
  volume: number
  high: number               // halalas
  low: number                // halalas
  open: number               // halalas
  previousClose: number      // halalas
  timestamp: string
}
